...
import { Instagram, Globe, MessageCircle, Star, ArrowRight, X, ChevronLeft } from "lucide-react";
...
export default function NewGenMenu({ restaurant, categories, wheelItems }: NewGenMenuProps) {
  ...
  // Scroll olayını dinleyip aktif kategoriyi güncelleme
  useEffect(() => {
    if (!isMenuOpen) return;

    const handleScroll = () => {
      // Basit bir spy scroll mantığı
                        ...
                        {cat.imageUrl && (
                            <div className="relative w-full h-40 md:h-52 rounded-xl overflow-hidden mb-6 shadow-sm border border-gray-100 dark:border-gray-800">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-4">
                                     {/* İstenirse buraya kategori açıklaması da yazılabilir */}
                                </div>
                            </div>
                        )}
